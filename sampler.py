...
class data_sampler(object):
    ...
    def __init__(self, config=None, seed=None, tokenizer=None, finetune=False, previous=False):

        self.config = config

        self.tokenizer = tokenizer

        # use all history training data
    ...
    def __next__(self):
        ...
        previous_relations = copy.deepcopy(self.seen_relations)
        previous_test_data = copy.deepcopy(self.history_test_data)
        current_relations = []
        cur_training_data = {}
        cur_valid_data = {}
        cur_test_data = {}
        ...
        for index in indices:
            ...
            self.history_train_data[self.id2rel[index]] = self.training_dataset[index]

        if self.finetune:
            return cur_training_data, cur_valid_data, cur_test_data, current_relations, self.history_train_data, self.history_test_data, self.seen_relations
        if self.previous:
            return cur_training_data, cur_valid_data, cur_test_data, current_relations, self.history_test_data, self.seen_relations, previous_test_data, previous_relations
